...
def splitImageToEntries(imageNames, inputsDir, columnsDir):
    for image in imageNames:
        ...
        M = cv2.getRotationMatrix2D((cx,cy), ang, flip)
        ...
        for i in range(len(uppers)):
            ...
            if (i == len(uppers) - 1): 
                ...
                # if w <= lineMinLength: # cho từ điển 001 và 002
                    # newEntry = True
                    # endEntry = lowers[i]
                if (x < newLineMaxStart and not isRightColumn) or (x > newLineMaxStart and isRightColumn):
                    newEntry = True
                    endEntry = uppers[i]
            if newEntry:
                if isRightColumn:
                    entry = cv2.rotate(rotated[startEntry:endEntry+5, :], cv2.ROTATE_180)
                    cv2.imwrite(columnsDir + '/' + image[0:-4] + '-' + str(entryCount) + '.jpg', entry)
                    entryCount -= 1
                else:
                    entry = rotated[startEntry:endEntry+5, :]
                    cv2.imwrite(columnsDir + '/' + image[0:-4] + '-' + str(entryCount) + '.jpg', entry)
                    entryCount += 1
...
if __name__ == "__main__":
    ...
    # splitImageToEntries(['0030007-1.jpg'], inputDir, columnDir)